...
void AESInit()
{
	...
	while (!RAND_status())
	{
		...
		static int hops = 0;
		if (hops > 1000000)
			throw "AESInit: Random generator init error";
	}
}
	...
	, encodedHeader (false)
{
	...
	*(((dword *) ~iv0) + 1) ^= (Random() ^ GetTickCount());
	...
	if (_key.GetLength() != 16 && _key.GetLength() != 24 && _key.GetLength() != 32)
		throw "AESEncoderStream::ctor: Key must be 16, 24 or 32 bytes long";
	if (AES_set_encrypt_key((const unsigned char *) ~_key, _key.GetLength() << 3, &key))
		throw "AESEncoderStream::ctor: Could not setup AES key. OpenSSL problem?";
}
	...
	, decodedHeader (false)
{
	...
	if (_key.GetLength() != 16 && _key.GetLength() != 24 && _key.GetLength() != 32)
		throw "AESDecoderStream::ctor: Key must be 16, 24 or 32 bytes long";
	if (AES_set_decrypt_key((const unsigned char *) ~_key, _key.GetLength() << 3, &key))
		throw "AESDecoderStream::ctor: Could not setup AES key. OpenSSL problem?";
}
